...
function Account({customer, heading, featuredDataPromise}: AccountType) {
  ...
  return (
    <>
      ...
      {orders && (
        <AccountOrderHistory orders={orders} cancelLink={logoutPath} />
      )}
...
type AccountOrderHistoryProps = OrderCardsProps & {
  cancelLink: string;
};

function AccountOrderHistory({orders, cancelLink}: AccountOrderHistoryProps) {
  return (
    <div className="mt-6">
      <div className="grid w-full gap-4 p-4 py-6 md:gap-8 md:p-8 lg:p-12">
        ...
        {orders?.length ? (
          ...
          <EmptyOrders cancelLink={cancelLink} />
        )}
      </div>
    </div>
  );
...
function EmptyOrders({cancelLink}: {cancelLink: string}) {
  return (
    <div>
